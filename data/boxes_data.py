...
def maybe_preprocess(config, data_path, sample_path=None):
    ...
    # if True:
        ...
        print("[*] Preprocessing real `boxes` data...")

        real_images = []
        for file in os.listdir('data/boxes/real_boxes_depth'):
            # print("!!",file)
            filepath = os.path.join('data/boxes/real_boxes_depth',file)
    ...
    for synthetic_image_path in synthetic_image_path_candidates:
        jpg_paths = glob(os.path.join(synthetic_image_path, '*.png'))
        # cropped_jpg_paths = glob(os.path.join(
        #     synthetic_image_path, '*_cropped.png'))
        #
        if len(jpg_paths) == 0:
        ...
        #             format(len(jpg_paths), max_synthetic_num)
                ...
                #     print(file)
                #     v_img = cv2.imread(file)
                #     # cv2.imwrite(file,v_img)
                #     # v_img = cv2.imread(file)
                #     print(v_img.shape)
...
def load(config, data_path, sample_path, rng):
    ...
    boxes_data = np.load(os.path.join(data_path, DATA_FNAME))
    real_data = boxes_data['real']
...
class DataLoader(object):
    def __init__(self, config, rng=None):
        ...
        self.data_path = os.path.join(config.data_dir, 'boxes')
        self.sample_path = os.path.join(self.data_path, config.sample_dir)
        self.batch_size = config.batch_size
        self.debug = config.debug

        self.real_data, synthetic_image_path = load(
            config, self.data_path, self.sample_path, rng)

        self.synthetic_data_paths = np.array(
            glob(os.path.join(synthetic_image_path, '*.png')))
